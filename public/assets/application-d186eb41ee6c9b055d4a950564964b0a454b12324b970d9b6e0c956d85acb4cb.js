...
(function() {
  $(document).on('click', '.submit-btn', function() {
    ...
    return $('.split-confirm').on('click', function() {
      var i, len, part, parts, results, split;
      element.hide();
      split = $('#split').val();
      if (split === 'space') {
        split = ' ';
      ...
      for (i = 0, len = parts.length; i < len; i++) {
        part = parts[i];
        results.push(element.after(createPattern(part)));
      }
      return results;
    });
  });
...
(function() {
  ...
  $(document).on('ready page:load', function() {
    return $('.table tr:visible td:nth-child(2)>div>input').on("keyup", function() {
      var values;
      values = [];
      $('.table tr:visible td:nth-child(2)>div>input').each(function() {
      ...
      if ($.inArray('<UNKNOWN>', values) > -1) {
        ...
        return $('[data-toggle="tooltip"]').removeAttr('title');
      }
    });
  });

  window.group = function() {
    var button, checked, first, sign, values;
  ...
  window.pat = function() {
    ...
    values = [];
    $('.table tr:visible td:nth-child(2)>div>input').each(function() {
      var data, i, j, multiString, ref, results;
      multiString = $(this).val();
      data = multiString.split(" ");
      results = [];
      for (i = j = 0, ref = data.length; 0 <= ref ? j < ref : j > ref; i = 0 <= ref ? ++j : --j) {
        results.push(values.push("%{" + data[i].toString() + "}"));
      }
      return results;
    });
    ...
    return $('#pattern_text').val(values.join(separator));
  };

  $(document).on('ready page:load', function() {
    return $(".group").on("click", function() {
      if ($("#table .analyzer tr input:checked").length === 0) {
        return $('#g-arrow').fadeIn(1000);
      }
    });
  });

  $(document).on('ready page:load', function() {
    return $("#help_submit").on("click", function() {
  ...
  $(document).on('ready page:load', function() {
    return $(".arrow-down").on("click", function() {
      ...
      if ($(this).closest('tr').next().attr('class') === className) {
        return $(this).closest('tr').next().fadeToggle(1000);
      }
    });
  });
  ...
  $(document).on('ready page:load', function() {
    ...
    return elements.each(function() {
      if ($(this).attr('class') !== '<UNKNOWN>') {
        return $(this).find("td:nth-child(2)>div>input").css('background-color', '#b2e8ae');
      }
    });
  });

  window.to_event = function() {
    var log_data, separator, values;
    values = [];
    $('.table tr:visible td:nth-child(2)>div>input').each(function() {
      var data, i, j, multiString, ref, results;
      multiString = $(this).val();
...
(function() {
  $(document).on('ready page:load', function() {
    return $('#event-arrow').fadeIn(1000);
  });

  window.event_pattern = function() {
    var values;
    values = [];
    $('.table tr td input').each(function() {
      return values.push($(this).val());
    });
    return $.ajax('/save_event', {
      type: "POST",
      dataType: "JSON",
      data: {
  ...
  $(document).on('ready page:load', function() {
    if ($('#warn').html() !== '') {
      return $('#event-options input').prop('disabled', true);
    }
  });

  window.event_tag = function() {
    return $('.output-tag').show(200);
  };
...
(function() {
  $(document).on('change', ".parsers", function() {
    var selectValue;
